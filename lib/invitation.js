...
exports.create = functions.database.ref('/invitations/{invitationId}').onCreate(snapshot => {
...
exports.accept = functions.database.ref('/invitations/{invitationId}').onUpdate(change => {
  if (change.after.child('user').exists()) {
    ...
    return admin.database().ref(`/invitations/${change.after.key}`).remove().then(() => {
